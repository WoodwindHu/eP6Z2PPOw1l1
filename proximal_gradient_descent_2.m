function [M, weight, pr] = proximal_gradient_descent_2(patch_s, patch_t, use_normal)
  ...
  %   used = false;
    ...
    %     weights = diag(exp(-(diff * M * diff')));
        weights = exp(-sum((diff * M) .* diff, 2));  % row sum
        cur_val = sum(weights .* dist);

        % 20190821
        weight = sum((diff * M) .* diff, 2); % Mahalanobis distance as a vector
        % 20190821
