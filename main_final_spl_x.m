...
%                     B = U'*L*U+lambda1(itr)*speye(length(X_pre));
                ...
                pc = pointCloud(X_rec, 'Color', pt_X.Color);
                name = [n_filename_ne 'xrec_' num2str(itr) '_' num2str(lambda1(itr)) '_' num2str(lambda2(itr)) '.ply'];
                pcwrite(pc, name);
...
%             write_ply_only_points(X_rec,[n_filename_ne '_dn.ply']);
              pcwrite(pc, [n_filename_ne '_dn.ply']);
